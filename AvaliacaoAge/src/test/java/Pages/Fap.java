...
public class Fap {
	...
	public String validarTelaFap() {
		...
		if(validar.getText().trim().contentEquals(("Gestão FAP: estratégiapara diminuir custos").toUpperCase().trim())) {
			Utils.log(Status.PASS, validar, "Acessar pagina do FAP");			
		}else {
			Utils.log(Status.FAIL, validar, "Erro ao acessar pagina do FAP. Esperado por '"+("Gestão FAP: estratégiapara diminuir custos").toUpperCase().trim()+"'.");			
		}
